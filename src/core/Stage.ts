...
export class Stage extends UIElement implements IStage {
    ...
    public setWindow(ownerWindow: Window) {
        ...
        let target = (container || doc) as EventTarget;
        target.addEventListener('pointerdown', ev => this.handlePointer(ev as MouseEvent, 0), { passive: false });
        target.addEventListener('pointerup', ev => this.handlePointer(ev as MouseEvent, 1), { passive: false });
        target.addEventListener('pointermove', ev => this.handlePointer(ev as MouseEvent, 2), { passive: false });
        target.addEventListener('pointercancel', ev => this.handlePointer(ev as MouseEvent, 3), { passive: false });
        target.addEventListener('pointerleave', ev => this.handlePointer(ev as MouseEvent, 3), { passive: false });
        target.addEventListener('contextmenu', ev => this.handleContextMenu(ev));

        target.addEventListener('dragend', ev => this.handlePointer(ev as DragEvent, 1), { passive: false });
        target.addEventListener('dragover', ev => this.handlePointer(ev as DragEvent, 2), { passive: false });
        target.addEventListener('wheel', ev => this.handleWheel(ev as WheelEvent), { passive: false });
        ...
        target = (container || ownerWindow) as EventTarget;
        target.addEventListener('keydown', ev => this.onKeydown(ev as KeyboardEvent));
        target.addEventListener('keyup', ev => this.onKeyup(ev as KeyboardEvent));
